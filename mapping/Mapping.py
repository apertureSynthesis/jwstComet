...
class Mapping(object):
    ...
    def makeMaps(self,cubeFiles,specStem,csvFile,waveLo,waveUp,radAp,name,objectType,composition,retrieval,key=None):
        """
    ...
    def plotMaps(self,csvFile):
        ...
        if (len(df_values.keys()) > 1):
            ...
            for i in range(len(df_values.keys())):
                ...
                plot_array = np.zeros(((df['X-Index'].max()),(df['Y-Index'].max())))
